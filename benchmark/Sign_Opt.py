...
class OPT_attack_sign_SGD(object):
    ...
    def attack_untargeted(self, x0, y0, init_adv, alpha=0.2, beta=0.001, iterations=20000000, query_limit=100000000,
                          distortion=None, svm=False, momentum=0.0):
        ...
        for i in range(iterations):
            intermediate.append([queries,torch.norm(torch.from_numpy(gg*xg).to(device)).item()])
            if queries>dba_attack_utilsV6.get_max_queries()or torch.norm(torch.from_numpy(gg*xg).to(device))<=dba_attack_utilsV6.get_threshold():
                break
            ...
            #    break
        dist=torch.norm(torch.from_numpy(gg*xg).to(device))
        intermediate.append([queries,dist.item()])
        if distortion is None or gg < distortion:
            target = model.predict_label(x0 + torch.tensor(gg * xg, dtype=torch.float).to(device))
            print("Succeed distortion {:.4f} target"
                  " {:d} queries {:d} LS queries {:d}\n".format(gg, target, query_count, ls_total))
            print('l2: %.4f'%(dist))
            return x0 + torch.tensor(gg * xg, dtype=torch.float).to(device), gg, True, query_count, xg,intermediate
        ...
        return x0 + torch.tensor(gg * xg, dtype=torch.float).to(device), gg, False, query_count, xg,intermediate
        ...
        for iii in range(K):  # for each u
            ...
            if (target is not None and
                    self.model.predict_label(
                        x0 + torch.tensor(initial_lbd * new_theta, dtype=torch.float).to(device)) == target):
                sign = -1

            # Untargeted case
            # preds.append(self.model.predict_label(x0+torch.tensor(initial_lbd*new_theta, dtype=torch.float).to(device)).item())
            if (target is None and
                    self.model.predict_label(
                        x0 + torch.tensor(initial_lbd * new_theta, dtype=torch.float).to(device)) != y0):  # success
                sign = -1
    ...
    def sign_grad_svm(self, x0, y0, theta, initial_lbd, h=0.001, K=100, lr=5.0, target=None):
        ...
        for iii in range(K):
            ...
            if (target is None and
                    self.model.predict_label(
                        x0 + torch.tensor(initial_lbd * new_theta, dtype=torch.float).to(device)) != y0):
                sign = -1
    ...
    def fine_grained_binary_search(self, model, x0, y0, theta, initial_lbd, current_best):
        ...
        if initial_lbd > current_best:
            if model.predict_label(x0 + torch.tensor(current_best * theta, dtype=torch.float).to(device)) == y0:
                nquery += 1
                return float('inf'), nquery
            lbd = current_best
        else:
...
class my_sign_opt:
    def __init__(self,model,samples,labels,input_device):
        ...
        self.sign_opt=OPT_attack_sign_SGD(my_model(model))
        self.labels=labels
    ...
    def attack(self):
        ...
        for sample,label,init_adv in zip(self.samples,self.labels,self.best_advs):
            ...
            init_adv=torch.reshape(init_adv,(3,get_side_length(),get_side_length()))

            perturbed,_,_,_,_,intermediate=self.sign_opt.attack_untargeted(sample,label,init_adv)
