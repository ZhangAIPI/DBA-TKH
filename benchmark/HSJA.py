...
def hsja(model,
    ...
    intermediate = []
    intermediate.append([0, compute_distance(sample,perturbed,constraint)])
    ...
    for j in np.arange(params['num_iterations']):
        ...
        dist = compute_distance(perturbed, sample, constraint)
        intermediate.append([model.queries,dist])
        if verbose:
            print('iteration: {:d}, {:s} distance {:f}'.format(j + 1, constraint, dist))
        if flag:
            return perturbed,intermediate
...
class my_hsja:
    def __init__(self,model,samples,input_device,original_labels=None):
        global device
        device = input_device
    ...
    def attack(self):
        ...
        criterion = get_criterion(self.original_labels)
        init_advs = init_attack.run(self.original_model, self.samples, criterion, early_stop=None)
        sample_adv_list=[]
        queries = []
        intermediates = []
        max_length = 0
        for sample,original_label,init_adv in zip(self.samples,self.original_labels,init_advs):
            ...
            perturbed,intermediate=hsja(model=self.model,sample=sample,original_label=original_label,init_adv=init_adv)
            sample_adv_list.append(perturbed)
            queries.append(self.model.queries)
            intermediate = np.array(intermediate)
            intermediates.append(intermediate)
            if max_length<intermediate.shape[0]:
                max_length = intermediate.shape[0]
        sample_adv_list=np.array(sample_adv_list)
        queries = np.array(queries)
        intermediates = np.array(intermediates)
        return torch.from_numpy(sample_adv_list).to(device).float(),queries,intermediates,max_length
