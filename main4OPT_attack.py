...
from benchmark import OPT_attack
...
if __name__ == "__main__":
    ...
    opt_model = OPT_attack.my_opt(fmodel, images, labels,input_device=device)
    opt_advs, opt_intermediates, max_length = opt_model.attack()
    print('OPT Attack Done')
    print("{:.2f} s to run".format(time.time() - time_start))
    print("Results")
    opt_labels_advs = fmodel(opt_advs.float()).argmax(1)
    opt_advs_l2 = l2(images, opt_advs)

    for image_i in range(len(images)):
        print("Adversarial Image {}:".format(image_i))
        label_o = int(labels[image_i])

        print("OPT Adversarial Image {}:".format(image_i))
        label_o = int(labels[image_i])
        label_adv = int(opt_labels_advs[image_i])
        print("\t- Original label: {}".format(imagenet_labels[str(label_o)]))
        print("\t- Adversarial label: {}".format(imagenet_labels[str(label_adv)]))
        print("\t- l2 = {}".format(opt_advs_l2[image_i]))

        ###############################
    print("Save Results")

    dba_attack_utilsV6.save_results(opt_intermediates, selected_paths, 'opt', args.n_images, max_length, )

    print("OPT:{}".format(torch.mean(opt_advs_l2)))
    print("we test %d pictures, and %d pictures are misclassified!" % (args.n_images, sum(opt_advs_l2 == 0)))
    '''for image_i, o in enumerate(images):
        adv_i = np.array(opt_advs[image_i].cpu().numpy() * 255).astype(np.uint8)
        img_adv_i = Image.fromarray(adv_i.transpose(1, 2, 0), mode="RGB")
        img_adv_i.save(os.path.join(output_folder, "opt_{}_adversarial.jpg".format(image_i)))'''
