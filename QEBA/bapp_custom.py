...
from .attack_base import call_decorator
...
sys.path.append("..")
from dba_attack_utilsV6 import get_max_queries
...
class BAPP_custom(Attack):
    ...
    * ability to specify the batch size
    ...
    The implementation in my_foolbox is based on Boundary Attack.
    """
    ...
    def __call__(
        ...
        self.queries = 0
        self.max_queries = get_max_queries()
    ...
    def attack(
        ...
        self.queries = 0
        a.queries=self.queries
        a.intermediate = []
        a.intermediate.append([0, dist.item()])
        ...
        for step in range(1, iterations + 1):
            ...
            # Log the step
            # ===========================================================
            # Using my_foolbox definition of distance for logging.
            if self.constraint == 'l2':
            ...
            a.intermediate.append([self.queries, np.linalg.norm(a.perturbed - a.unperturbed)])
            a.queries = self.queries
            if mask_succeed > 0:
                self.__mask_succeed = 1
                break
        ...
        # self.log_time()
        a.queries = self.queries
        a.intermediate = np.array(a.intermediate)
        # Save the labels
        if self.save_calls is not None:
    ...
    def initialize_starting_point(self, a):
        ...
        if starting_point is not None:
            ...
            return'''
            a.set_perturbed(starting_point)

        """
        Apply BlendedUniformNoiseAttack if without
        initialization.
        Efficient Implementation of BlendedUniformNoiseAttack in my_foolbox.
        """
        success = 0
        num_evals = 0
    ...
    def log_step(self, step, distance, message='', always=False, a=None, perturbed=None, update=None, aux_info=None):
        ...
        if a is not None:
            ...
            self.printv("Call:", a._total_prediction_calls, "Saved to",
                        'BAPP_result/%sstep%d.png' % (self.suffix, step))
