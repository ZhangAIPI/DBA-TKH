...
import sys
sys.path.append("..")
from dba_attack_utilsV6 import get_side_length
...
def load_imagenet_img(path):
    ...
    tmp = np.array(image)
    image = image.resize((get_side_length(),get_side_length()))
...
def imagenet_attack(args, N_img):
    ...
    fmodel = PyTorchModel(resnet18, bounds=(0, 1), num_classes=1000, preprocessing=(mean, std), discretize=args.model_discretize)
    ...
    mask = None
    #mask = np.zeros((3,get_side_length(),get_side_length())).astype(np.float32)
    ##mask[:,:56,:56] = 1  #H
    ##mask[:,-56:,-56:] = 1  #H
    ##mask[:,:56,-56:] = 1  #E
    ##mask[:,-56:,:56] = 1  #E
...
def cifar_attack(args, N_img):
    ...
    model.load_state_dict(torch.load('../pretrained_models/cifar10_res18.model'))
    mean = np.array([0.485, 0.456, 0.406])
    std = np.array([0.229, 0.224, 0.225])
    fmodel = PyTorchModel(model, bounds=(0, 1), num_classes=10, preprocessing=(mean.reshape((3,1,1)), std.reshape((3,1,1))))
...
def celeba_attack(args, N_img):
    ...
    model.load_state_dict(torch.load('../pretrained_models/celeba.model'))
    mean = np.array([0, 0, 0])
    std = np.array([1, 1, 1])
    fmodel = PyTorchModel(model, bounds=(0, 1), num_classes=10, preprocessing=(mean.reshape((3,1,1)), std.reshape((3,1,1))))
    ...
    import torchvision.transforms as transforms
    transform = transforms.Compose([
        transforms.Resize((get_side_length(), get_side_length())),
        transforms.ToTensor()
    ])
...
def imagenet_attack_old(args, pgen_type):
    ...
    mean = np.array([0.485, 0.456, 0.406]).reshape((3, 1, 1))
    std = np.array([0.229, 0.224, 0.225]).reshape((3, 1, 1))
    fmodel = PyTorchModel(resnet18, bounds=(0, 1), num_classes=1000, preprocessing=(mean, std))
