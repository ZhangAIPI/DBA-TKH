...
class Model(ABC):
    """Base class to provide attacks with a unified interface to pretrained_models.
...
class DifferentiableModel(Model):
    """Base class for differentiable pretrained_models.

    The :class:`DifferentiableModel` class can be used as a base class for pretrained_models that can support
