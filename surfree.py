...
class SurFree(MinimizationAttack):
    ...
    def run(
        ...
        if not self._is_adversarial(best_advs).all():
            print('Can not find an adversarial point')
            global intermediate
            intermediate.append([dba_attack_utilsV6.get_max_queries(),dba_attack_utilsV6.get_num()])
        ...
        for _ in range(self._steps):
            # global intermediate
            intermediate.append([self.get_nqueries()[0], torch.norm(best_advs.raw - originals.raw).item()])
            ...
            if all(v > dba_attack_utilsV6.get_max_queries() for v in self._nqueries.values()) or torch.norm(
                    best_advs.raw - originals.raw) < dba_attack_utilsV6.get_threshold():
...
def once_attack(f_attack, fmodel, images, labels, config):
    intermediates = []
    ...
    for i in range(images.shape[0]):
        ...
        image = images[i:i + 1]
        label = labels[i:i + 1]
        element, adv, success = f_attack(fmodel, image, label, **config["run"])
        # global intermediate
    ...
    return advs, intermediates, max_length
