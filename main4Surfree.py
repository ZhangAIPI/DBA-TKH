...
if __name__ == "__main__":
    ...
    advs, intermediates, max_length = surfree.once_attack(f_attack, fmodel, images, labels, config)
    print("{:.2f} s to run".format(time.time() - time_start))
    print("Results")
    labels_advs = fmodel(advs).argmax(1)
    nqueries = f_attack.get_nqueries()
    advs_l2 = l2(images, advs)
    ...
    for image_i in range(len(images)):
        ...
        # print("\t- l_inf = {}".format(advs_l_inf[image_i]))
        # print("\t- {} queries\n".format(nqueries[image_i]))

        ###############################
    print("Save Results")

    dba_attack_utilsV6.save_results(intermediates, selected_paths, 'surfree', args.n_images, max_length)

    print("baseline:{}".format(torch.mean(advs_l2)))
    print("we test %d pictures, and %d pictures are misclassified!" % (args.n_images, sum(advs_l2 == 0)))
